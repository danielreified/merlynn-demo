# ═══════════════════════════════════════════════════════════════
# Merlynn Risk Monitor — Production Stack
# ═══════════════════════════════════════════════════════════════
...
module "vpc" {
  ...
  private_subnet_b_cidr = "10.20.11.0/24"
...
module "ecs_web" {
  ...
  container = {
    ...
    environment = {
      ...
      NEXTAUTH_URL = "https://prod-app.${local.root_domain}"
    }
    secrets = [
      {
        name      = "MONGODB_URI"
...
module "cloudfront_storybook" {
  ...
  aliases                        = ["prod-storybook.${local.root_domain}"]
...
module "cloudfront_emails" {
  ...
  aliases                        = ["prod-emails.${local.root_domain}"]
  acm_certificate_arn            = module.acm_certs.cloudfront_cert_arn
  default_root_object            = "index.html"
  enable_spa_fallback            = false
  viewer_request_function_arn    = aws_cloudfront_function.spa_rewrite.arn
...
# ─── app.merlynn.co.za → ALB ──────────────────────────────
resource "aws_route53_record" "app" {
  zone_id = data.aws_route53_zone.root.zone_id
  name    = "prod-app.${local.root_domain}"
  type    = "A"
...
# ─── storybook.merlynn.co.za → CloudFront ─────────────────
resource "aws_route53_record" "storybook" {
  zone_id = data.aws_route53_zone.root.zone_id
  name    = "prod-storybook.${local.root_domain}"
  type    = "A"
...
# ─── emails.merlynn.co.za → CloudFront ────────────────────
resource "aws_route53_record" "emails" {
  zone_id = data.aws_route53_zone.root.zone_id
  name    = "prod-emails.${local.root_domain}"
  type    = "A"
